...
int main()
{
    ...
    while (1) // Infinite loop to repeatedly process user input until termination condition is met.
    {
        ...
        // Ask the user for the number of bits to shift the input number to the left.
        printf("How many bits you want to shift left?: ");
        scanf("%d", &x);

        // Perform left bitwise shift operation on 'n' by 'x' bits and store the result in 'm'.
        m = n << x;
